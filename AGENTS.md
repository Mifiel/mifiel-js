...
- Running the Stencil build/`start` **regenerates committed wrapper proxy files** (`packages/widget-react/lib/components/stencil-generated/components.ts` and `packages/widget-angular/src/lib/stencil-generated/components.ts`). These regenerated diffs are a byproduct of the current source and are unrelated to your change — revert them (`git checkout <file>`) before committing unless you intentionally changed the component's props/events.
